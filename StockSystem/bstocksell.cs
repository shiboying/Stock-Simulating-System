...
{
    public partial class bstocksell : Form
    {
        public bstocksell()
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (StockSystem.Spebra.otheruser[9].getBstock() >= amount)
            {
                StockSystem.Spebra.otheruser[9].setmoney(yue);
                StockSystem.Spebra.otheruser[9].setBstock(StockSystem.Spebra.otheruser[9].getBstock() + amount);
                MessageBox.Show("交易成功！交易价格为： " + dealprice + "\n您的账户余额为: " + yue + "\n您所持有的B股票数为： " + Convert.ToString(StockSystem.Spebra.otheruser[9].getBstock()));
